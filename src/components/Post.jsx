...
export default function Post({ postData, avatar, userData }) {
	...
	return (<>
		<div className="post_wrapper" data-pid={pid}>
			...
			{caption ? <ReactMarkdown className="caption">
				{caption}
			</ReactMarkdown> : null}
			{url ? <LazyLoad><img src={url} alt="image" className="post_image" /></LazyLoad> : null}
			...
			<div className="post_action">
				...
				<div className="commentbtn">
					...
					<div data-pid={pid} onClick={saveComment} ><i data-pid={pid} className="fas fa-paper-plane" id="postcomment" style={{ color: comment ? "#0038ef" : "#afd4ff" }}></i></div>
				</div>
			</div>
			{postComments.length > 0 ?
				<div className="commentbox">
					<div className="toggleComments"onClick={()=>setShowComments(!showComments)}>
						{showComments?<span>Hide Comments <i className="fas fa-chevron-up"/></span>:<span>Show comments <i className="fas fa-chevron-down"/></span>}
						</div>
					
					{showComments?postComments.map((val, i) => {
